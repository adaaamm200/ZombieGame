...
def measure(path, name, facing):
    ...
    no_gap_run = 0
    gap_run = 0
    seen_gap = False   # csak TARTOS lab-res utan szamolunk "osszeertek"-et
    for y in range(bottom - 5, floor_limit, -1):
        if y not in prof:
            continue
        if has_leg_gap(y):
            gap_run += 1
            # A legaljan a labujjak kozott is lehet 1-2 soros HAMIS res -> csak a
            # tartos (>=10 soros) rest fogadjuk el valodi lab-szetvalasnak.
            if gap_run >= 10:
                seen_gap = True
            no_gap_run = 0
            continue
        gap_run = 0          # megszakadt a res -> ujra kell epiteni a tartossagot
        if seen_gap:
            # A legaljan gyakran mar csak az EGYIK labfej latszik -> ott sincs res,
            # de az nem a csipo. Ezert csak a legalso valodi lab-res FOLOTT szamolunk.
            no_gap_run += 1
    ...
    # --- ANCHOR: talp (a jatek konvencioja: origo = talp, x = VIZSZINTES KOZEP) ---
    # NEM a hip_x-et hasznaljuk: a csipo-res detektalasa felrecsuszhat (boss/brute),
    # es akkor az egesz alak oldalra tolodna a talp-pontjahoz kepest. A sziluett
    # kozepe a helyes referencia (a mukodo Kobornal ez 0.3px-re egyezik).
    anchor_x = (min(p[0] for p in prof.values()) + max(p[1] for p in prof.values())) // 2
    anchor_y = bottom
